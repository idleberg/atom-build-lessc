  ...
  return class LesscProvider {
    ...
    isEligible() {
      try {
        ...
        if (atom.inDevMode()) atom.notifications.addInfo(`**${meta.name}**`, { detail: e, dismissable: false });
        return true;
      } catch (error) {
        if (atom.inDevMode()) atom.notifications.addError(notEligible, { detail: stdout, dismissable: true });
        return false;
      }
    }
