...
export function provideBuilder() {
  return class LesscProvider extends EventEmitter {
    ...
    settings() {
      ...
      // User settings
      const outputFile = atom.config.get('build-lessc.outputFile');
      const customArguments = atom.config.get('build-lessc.customArguments');
      ...
      return [
        ...
        },
        {
          name: 'Less (user)',
          exec: 'lessc',
          args: [ customArguments, '{FILE_ACTIVE}', outputFile ],
          cwd: '{FILE_ACTIVE_PATH}',
          sh: false,
          atomCommandName: 'less:compile',
          errorMatch: errorMatch
